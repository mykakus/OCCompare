...
import trimesh
import torch
import pyfqmr # reikis grazinti sita versija
import numpy as np
...
#----Downsample
mesh_simple,labels=Downsample(mesh)
...
for i in range(len(np.unique(pre_labels))):
    ...
    mesh_sub_par=test_mesh_predicted.submesh([matches_first_color],only_watertight =False) 
    mesh_sub_par[0].remove_unreferenced_vertices()
    
    Part_extract=SDF(mesh_sub_par[0].vertices, mesh_sub_par[0].faces) #SDF
    Ext_scal=Part_extract(mesh_OG.vertices)
    Vertices_trim=mesh_OG.vertices[(Ext_scal>-0.4) & (Ext_scal<0.4)]
    ID_vert=mesh_OG.vertex_faces[np.all(np.isin(mesh_OG.vertices,Vertices_trim ), axis=1)] 
    Extr_part=mesh_OG.submesh([np.unique(ID_vert)[1:]],append=True)
...
#----Overal part 
for j in range(len(Store_segments)):
    if j!=len(Store_segments)-1:
        Concat=sum([Store_segments[i] for i in range(j+1,len(Store_segments))])
        Concat.update_faces(Concat.unique_faces())
        Concat.remove_unreferenced_vertices()
       
        Kur_persidengimas = np.where(~np.isin(Store_segments[j].vertices, Concat.vertices).all(axis=1))[0]
        contains_vertices = np.array([np.isin(face, Kur_persidengimas).any() for face in Store_segments[j].faces])
        Store_segments[j].update_faces(contains_vertices)
        Store_segments[j].remove_unreferenced_vertices()
...
for jj in Store_segments:
    ps.register_surface_mesh(jj.metadata['file_name'], jj.vertices, jj.faces,color=jj.metadata['color'],enabled =False)
ps.show()
ps.remove_all_structures()
